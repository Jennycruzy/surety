...
import * as local from "@surety/txline-verify";
...
try {
  ...
  await exec("npm", ["install", `@surety/txline-verify@${version}`, "--registry=https://registry.npmjs.org", "--ignore-scripts"], { cwd: directory });
  const published = await import(pathToFileURL(path.join(directory, "node_modules/@surety/txline-verify/dist/index.js")).href);
  ...
  console.log(`PASS: public @surety/txline-verify@${version} matched in-repo result ${actual}`);
